...
class PredictionSystem:
    """Handles Big Brother prediction polls and leaderboard"""
    ...
    def init_prediction_tables(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS leaderboard (
                ...
                hoh_correct INTEGER DEFAULT 0,
                veto_correct INTEGER DEFAULT 0,
                eviction_correct INTEGER DEFAULT 0,
        ...
        # Create indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_poll_active ON polls(is_active)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_prediction_poll ON predictions(poll_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_leaderboard_points ON leaderboard(total_points DESC)")
    ...
    def setup_commands(self):
        ...
        async def commands_slash(interaction: discord.Interaction):
            ...
            commands_list = [
                ...
                ("/vote", "Cast your prediction vote"),
                ("/polls", "Show all active prediction polls"),
                ("/closepoll", "Close a prediction poll (Admin only)"),
                ("/setresult", "Set poll result and award points (Admin only)"),
                ("/extendpoll", "Extend poll closing time (Admin only)"),
                ("/leaderboard", "Show top 10 predictors"),
                ("/mystats", "Show your prediction statistics")
            ]
        ...
        async def create_poll_slash(interaction: discord.Interaction, 
            ...
            try:
                ...
                # Validate poll type
                valid_types = ['HOH', 'VETO', 'EVICTION']
                if poll_type.upper() not in valid_types:
                    await interaction.followup.send(f"Invalid poll type! Use one of: {', '.join(valid_types)}", ephemeral=True)
                    return
                
                # Parse options
                option_list = [opt.strip() for opt in options.split(',') if opt.strip()]
                if len(option_list) < 2:
                    await interaction.followup.send("You need at least 2 options for a poll!", ephemeral=True)
                    return
                
                # Create poll
                success, message, poll_id = self.prediction_system.create_poll(
                    poll_type=poll_type,
                    week=week,
                    options=option_list,
                    duration=duration,
                ...
                if success:
                    ...
                    embed = discord.Embed(
                        title=f"🗳️ New {poll_type.upper()} Prediction Poll!",
                        description=f"**Week {week} {poll_type.upper()} predictions are now open!**\n\n"
                                   f"Use `/vote {poll_id} [your choice]` to cast your prediction",
                        color=0x3498db,
                        timestamp=datetime.now()
                    )
                    ...
                    await interaction.followup.send(f"✅ Poll created! ID: {poll_id}", ephemeral=True)
                else:
        ...
        @self.tree.command(name="vote", description="Cast your prediction vote")
        async def vote_slash(interaction: discord.Interaction, poll_id: int, choice: str):
            """Cast a vote"""
            try:
                await interaction.response.defer()
                
                success, message = self.prediction_system.cast_vote(
                    poll_id=poll_id,
                ...
                await interaction.followup.send("Error recording vote")

        @self.tree.command(name="polls", description="Show all active prediction polls")
        async def polls_slash(interaction: discord.Interaction):
            ...
            try:
                ...
                embed = self.prediction_system.create_poll_embed(polls)
        ...
        @self.tree.command(name="closepoll", description="Close a prediction poll (Admin only)")
        async def close_poll_slash(interaction: discord.Interaction, poll_id: int):
            ...
            try:
                ...
                success, message = self.prediction_system.close_poll(poll_id)
        ...
        @self.tree.command(name="setresult", description="Set poll result and award points (Admin only)")
        async def set_result_slash(interaction: discord.Interaction, poll_id: int, winner: str):
            """Set poll result"""
            try:
                if not interaction.user.guild_permissions.administrator:
                    await interaction.response.send_message("You need administrator permissions to use this command.", ephemeral=True)
                ...
                success, message, result_data = self.prediction_system.set_poll_result(poll_id, winner.strip())
                
                if success:
                    # Create result announcement embed
                    embed = discord.Embed(
                        title="🎉 Prediction Results!",
                        description=f"**{result_data['winner']}** wins!",
                        color=0x2ecc71,
                        timestamp=datetime.now()
                    )
                    ...
                    embed.add_field(
                        name="📊 Results",
                        value=f"**{result_data['correct_count']}** people guessed correctly\n"
                              f"**{result_data['points']}** points awarded to each",
                        inline=False
                    )
                    
                    # List some winners
                    if result_data['winners']:
                        winner_names = [w[1] for w in result_data['winners'][:5]]
                        winners_text = "\n".join([f"• {name}" for name in winner_names])
                        if len(result_data['winners']) > 5:
                            winners_text += f"\n• And {len(result_data['winners']) - 5} more..."
                        
                        embed.add_field(
                            name="🏆 Correct Predictors",
                            value=winners_text,
                            inline=False
                        )
                ...
                logger.error(f"Error setting result: {e}")
                await interaction.followup.send("Error setting result")
        ...
        async def leaderboard_slash(interaction: discord.Interaction):
            ...
            try:
                ...
                leaderboard = self.prediction_system.get_leaderboard(limit=10)
                embed = self.prediction_system.create_leaderboard_embed(leaderboard)
        ...
        @self.tree.command(name="mystats", description="Show your prediction statistics")
        async def my_stats_slash(interaction: discord.Interaction):
            """Show user stats"""
            try:
                await interaction.response.defer()
                
                stats = self.prediction_system.get_user_stats(
                    user_id=str(interaction.user.id),
                    user_name=interaction.user.display_name
                )
                
                embed = self.prediction_system.create_user_stats_embed(stats, interaction.user.display_name)
                
                await interaction.followup.send(embed=embed)
                
            except Exception as e:
                logger.error(f"Error showing stats: {e}")
                await interaction.followup.send("Error retrieving stats")

        @self.tree.command(name="extendpoll", description="Extend poll closing time (Admin only)")
        async def extend_poll_slash(interaction: discord.Interaction, poll_id: int, additional_time: str):
            """Extend poll time"""
            try:
                if not interaction.user.guild_permissions.administrator:
                    await interaction.response.send_message("You need administrator permissions to use this command.", ephemeral=True)
                ...
                duration_td = self.prediction_system.parse_duration(additional_time)
                if not duration_td:
                    await interaction.followup.send("Invalid duration format! Use: 30m, 2h, 1d, etc.", ephemeral=True)
                    return
                
                # Get current poll
                conn = self.prediction_system.get_connection()
                cursor = conn.cursor()
                
                cursor.execute("SELECT closes_at, is_active FROM polls WHERE poll_id = ?", (poll_id,))
                result = cursor.fetchone()
                ...
                # Calculate new closing time
                if closes_at:
                    current_close = datetime.fromisoformat(closes_at)
                    new_close = current_close + duration_td
                else:
                    new_close = datetime.now() + duration_td
                
                # Update poll
                cursor.execute("UPDATE polls SET closes_at = ? WHERE poll_id = ?", (new_close, poll_id))
                conn.commit()
                conn.close()
                
                await interaction.followup.send(f"✅ Poll extended by {additional_time}! New close time: {new_close.strftime('%Y-%m-%d %I:%M %p')}", ephemeral=True)
                
            except Exception as e:
                logger.error(f"Error extending poll: {e}")
                await interaction.followup.send("Error extending poll", ephemeral=True)
    ...
    async def on_ready(self):
        ...
        try:
            ...
            self.check_polls_task.start()  # Start poll checking task
            logger.info("RSS feed monitoring, daily recap, and poll checking tasks started")
        except Exception as e:
    ...
    async def check_polls_task(self):
        """Check for expired polls and send reminders"""
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE is_active = 1 AND closes_at IS NOT NULL 
                AND datetime(closes_at) < datetime(?)
                RETURNING poll_id, poll_type, week_number
            ...
            if closed_polls:
                ...
                if channel_id:
                    ...
                    if channel:
                        for poll_id, poll_type, week in closed_polls:
                            embed = discord.Embed(
                                title="🔒 Poll Closed",
                                description=f"**{poll_type} Week {week}** predictions are now closed!",
                                color=0xe74c3c,
                                timestamp=datetime.now()
                            )
                            embed.set_footer(text=f"Poll #{poll_id}")
            ...
            # Check for polls closing within 1 hour
            one_hour_from_now = datetime.now() + timedelta(hours=1)
            cursor.execute("""
                SELECT poll_id, poll_type, week_number, closes_at
                FROM polls
                WHERE is_active = 1 
                ...
                AND datetime(closes_at) <= datetime(?)
                AND NOT EXISTS (
                    SELECT 1 FROM poll_reminders 
                    WHERE poll_reminders.poll_id = polls.poll_id
                )
            """, (datetime.now(), one_hour_from_now))
            
            reminder_polls = cursor.fetchall()
            ...
            if reminder_polls:
                ...
                if channel_id:
                    channel = self.get_channel(channel_id)
                    if channel:
                        for poll_id, poll_type, week, closes_at in reminder_polls:
                            time_left = datetime.fromisoformat(closes_at) - datetime.now()
                            minutes_left = int(time_left.total_seconds() / 60)
                            
                            embed = discord.Embed(
                                title="⏰ Poll Closing Soon!",
                                description=f"**{poll_type} Week {week}** closes in {minutes_left} minutes!\n\n"
                                           f"Use `/vote {poll_id} [choice]` to get your prediction in!",
                                color=0xf39c12,
                                timestamp=datetime.now()
                            )
                            embed.set_footer(text=f"Poll #{poll_id}")
                            ...
                            # Mark as reminded
                            cursor.execute("""
                                INSERT INTO poll_reminders (poll_id, reminded_at)
                                VALUES (?, ?)
                            """, (poll_id, datetime.now()))
            
            conn.commit()
            conn.close()
            
        except Exception as e:
            logger.error(f"Error in poll check task: {e}")
    
    @check_polls_task.before_loop
    async def before_check_polls_task(self):
        """Wait for bot to be ready before starting poll check task"""
...
if __name__ == "__main__":
    ...
    def create_poll(self, poll_type: str, week: int, options: List[str], 
                   duration: Optional[str], created_by_id: str, created_by_name: str) -> Tuple[bool, str, Optional[int]]:
        """Create a new poll"""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        try:
            # Check if poll already exists for this type/week
            cursor.execute("""
                SELECT poll_id FROM polls 
                WHERE poll_type = ? AND week_number = ? AND season = 27 AND is_active = 1
            """, (poll_type.upper(), week))
            
            if cursor.fetchone():
                return False, f"A {poll_type} poll for week {week} already exists!", None
            ...
            if duration:
                ...
                if duration_td:
                    closes_at = datetime.now() + duration_td
                else:
                    return False, f"Invalid duration format. Use formats like: 30m, 2h, 1d, 1w", None
            
            # Create question
            questions = {
                'HOH': f"Who will win HOH in Week {week}?",
                'VETO': f"Who will win the Power of Veto in Week {week}?",
                'EVICTION': f"Who will be evicted in Week {week}?"
            }
            question = questions.get(poll_type.upper(), f"{poll_type} - Week {week}")
            
            # Insert poll
            cursor.execute("""
                INSERT INTO polls (poll_type, week_number, question, options, created_at, closes_at, created_by_id, created_by_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (poll_type.upper(), week, question, json.dumps(options), datetime.now(), closes_at, created_by_id, created_by_name))
            
            poll_id = cursor.lastrowid
            conn.commit()
            
            return True, "Poll created successfully!", poll_id
            
        except Exception as e:
            logger.error(f"Error creating poll: {e}")
            return False, "Error creating poll", None
        finally:
            conn.close()
    
    def cast_vote(self, poll_id: int, user_id: str, user_name: str, choice: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT options, is_active, closes_at FROM polls WHERE poll_id = ?
            """, (poll_id,))
            ...
            options_json, is_active, closes_at = result
            ...
            if choice not in options:
                return False, f"Invalid choice! Options are: {', '.join(options)}"
            
            # Insert or update vote
            cursor.execute("""
                INSERT INTO predictions (user_id, user_name, poll_id, prediction, created_at)
                VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(user_id, poll_id) DO UPDATE SET
                    prediction = excluded.prediction,
                    created_at = excluded.created_at
            """, (user_id, user_name, poll_id, choice, datetime.now()))
            
            conn.commit()
            return True, f"Your vote for **{choice}** has been recorded!"
            
        except Exception as e:
            logger.error(f"Error casting vote: {e}")
            return False, "Error recording vote"
        finally:
            conn.close()
    
    def close_poll(self, poll_id: int) -> Tuple[bool, str]:
        """Close a poll"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            ...
            if cursor.rowcount == 0:
                return False, "Poll not found or already closed!"
            ...
            logger.error(f"Error closing poll: {e}")
            return False, "Error closing poll"
        finally:
            conn.close()
    
    def set_poll_result(self, poll_id: int, winner: str) -> Tuple[bool, str, Dict]:
        """Set poll result and award points"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            # Get poll info
            cursor.execute("""
                SELECT poll_type, week_number, options, is_active FROM polls WHERE poll_id = ?
            """, (poll_id,))
            ...
            poll_type, week_number, options_json, is_active = result
            
            # Validate winner
            options = json.loads(options_json)
            if winner not in options:
                return False, f"Invalid winner! Options were: {', '.join(options)}", {}
            
            # Close poll if still active
            if is_active:
                cursor.execute("UPDATE polls SET is_active = 0, closed_at = ? WHERE poll_id = ?", 
                             (datetime.now(), poll_id))
            
            # Set correct answer
            cursor.execute("UPDATE polls SET correct_answer = ? WHERE poll_id = ?", (winner, poll_id))
            
            # Award points
            points_map = {'HOH': 5, 'VETO': 5, 'EVICTION': 3}
            points = points_map.get(poll_type, 3)
            
            # Update predictions with points
            cursor.execute("""
                UPDATE predictions 
                SET points_earned = ? 
                WHERE poll_id = ? AND prediction = ?
            """, (points, poll_id, winner))
            
            correct_count = cursor.rowcount
            
            # Get all correct predictors
            cursor.execute("""
                SELECT user_id, user_name FROM predictions 
                WHERE poll_id = ? AND prediction = ?
            """, (poll_id, winner))
            ...
            # Update leaderboard
            for user_id, user_name in winners:
                # Ensure user exists in leaderboard
                cursor.execute("""
                    INSERT INTO leaderboard (user_id, user_name, season)
                    VALUES (?, ?, 27)
                ...
                cursor.execute("""
                    ...
                    WHERE user_id = ?
                """.format(f"{poll_type.lower()}_correct", f"{poll_type.lower()}_correct"), 
                (points, user_id))
            ...
            # Reset streaks for incorrect predictions
            cursor.execute("""
                UPDATE leaderboard 
            ...
            self._check_perfect_weeks(cursor, week_number)
            
            conn.commit()
            
            return True, f"Results set! {correct_count} people correctly predicted {winner}!", {
                'winner': winner,
            ...
            logger.error(f"Error setting result: {e}")
            conn.rollback()
            return False, "Error setting result", {}
        finally:
            conn.close()
    
    def _check_perfect_weeks(self, cursor, week_number: int):
        """Check and award perfect week bonuses"""
        # Find users who got all 3 predictions correct this week
        cursor.execute("""
            SELECT p.user_id, COUNT(DISTINCT pl.poll_type) as correct_types
            FROM predictions p
            JOIN polls pl ON p.poll_id = pl.poll_id
            WHERE pl.week_number = ? 
                AND pl.season = 27
                AND p.points_earned > 0
            GROUP BY p.user_id
            HAVING correct_types = 3
        """, (week_number,))
        
        perfect_users = cursor.fetchall()
        
        for user_id, _ in perfect_users:
            cursor.execute("""
                UPDATE leaderboard 
                SET total_points = total_points + 5,
                    perfect_weeks = perfect_weeks + 1
                WHERE user_id = ?
            """, (user_id,))
    ...
    def get_active_polls(self) -> List[Dict]:
        ...
        try:
            ...
            # Get active polls
            cursor.execute("""
                SELECT poll_id, poll_type, week_number, question, options, closes_at
                FROM polls 
                WHERE is_active = 1 AND season = 27
                ORDER BY week_number DESC, poll_type
            """)
            
            polls = []
            for row in cursor.fetchall():
                poll_id, poll_type, week_number, question, options_json, closes_at = row
                options = json.loads(options_json)
                ...
                for prediction, count in cursor.fetchall():
                    vote_counts[prediction] = count
                
                # Calculate time remaining
                time_remaining = None
                if closes_at:
                    remaining = datetime.fromisoformat(closes_at) - datetime.now()
    ...
    def get_leaderboard(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            cursor.execute("""
                SELECT user_name, total_points, correct_predictions, predictions_made,
                       perfect_weeks, current_streak, best_streak
                FROM leaderboard
                WHERE season = 27 AND total_points > 0
            ...
            leaderboard = []
            for row in cursor.fetchall():
                user_name, points, correct, made, perfect, streak, best = row
                accuracy = (correct / made * 100) if made > 0 else 0
    ...
    def get_user_stats(self, user_id: str, user_name: str) -> Dict:
        ...
        try:
            ...
            cursor.execute("""
                SELECT total_points, predictions_made, correct_predictions,
                       hoh_correct, veto_correct, eviction_correct,
                       perfect_weeks, current_streak, best_streak
                FROM leaderboard
            ...
            points, made, correct, hoh, veto, eviction, perfect, streak, best = result
            
            # Get recent predictions
            cursor.execute("""
                SELECT pl.poll_type, pl.week_number, p.prediction, p.points_earned, pl.correct_answer
                FROM predictions p
                JOIN polls pl ON p.poll_id = pl.poll_id
                WHERE p.user_id = ?
                ORDER BY pl.week_number DESC, pl.poll_type
                LIMIT 10
            """, (user_id,))
            
            recent = []
            for row in cursor.fetchall():
                poll_type, week, prediction, points_earned, correct_answer = row
                recent.append({
                    'type': poll_type,
                    'week': week,
    ...
    def create_poll_embed(self, polls: List[Dict]) -> discord.Embed:
        """Create embed showing active polls"""
        if not polls:
            embed = discord.Embed(
        ...
        embed = discord.Embed(
            title="🗳️ Active Prediction Polls",
            description="Cast your predictions with `/vote [poll_id] [choice]`",
            color=0x3498db,
            timestamp=datetime.now()
        )
        ...
        for poll in polls:
            ...
            if poll['time_remaining']:
                lines.append(f"⏰ Closes in {poll['time_remaining']}")
            ...
            sorted_options = sorted(
                poll['options'],
                key=lambda x: poll['vote_counts'].get(x, 0),
                reverse=True
            )
            ...
            for option in sorted_options:
                votes = poll['vote_counts'].get(option, 0)
                percentage = (votes / total_votes * 100) if total_votes > 0 else 0
                bar_length = int(percentage / 10)  # 10% = 1 block
                bar = "█" * bar_length + "░" * (10 - bar_length)
                lines.append(f"`{option:12}` {bar} {votes} ({percentage:.0f}%)")
            
            lines.append(f"\n**Poll ID: {poll['poll_id']}** • Total votes: {total_votes}")
            
            embed.add_field(
                name=f"{self._get_poll_emoji(poll['poll_type'])} Week {poll['week_number']} - {poll['poll_type']}",
                value="\n".join(lines),
                inline=False
            )
        
        embed.set_footer(text="Use /vote to cast your prediction!")
        return embed
    
    def create_leaderboard_embed(self, leaderboard: List[Dict]) -> discord.Embed:
        """Create leaderboard embed"""
        embed = discord.Embed(
            title="🏆 Prediction Leaderboard",
            description="Top Big Brother predictors this season",
            color=0xffd700,
            timestamp=datetime.now()
        )
        
        if not leaderboard:
            embed.add_field(name="No predictions yet", value="Be the first to make a prediction!", inline=False)
            return embed
        ...
        for i, user in enumerate(leaderboard):
            ...
            # Create stats line
            stats = f"**{user['total_points']}pts** • {user['accuracy']:.0f}% • "
            if user['current_streak'] > 2:
                stats += f"🔥{user['current_streak']} • "
            if user['perfect_weeks'] > 0:
                stats += f"✨{user['perfect_weeks']} "
            
            lines.append(f"{medal} **{user['user_name']}**\n   {stats}")
        ...
        embed.add_field(
            name="Legend",
            value="🔥 Current Streak • ✨ Perfect Weeks • pts = Points • % = Accuracy",
            inline=False
        )
        
        embed.set_footer(text="Predict with /vote • Check your stats with /mystats")
        return embed
    
    def create_user_stats_embed(self, stats: Dict, user_name: str) -> discord.Embed:
        """Create user statistics embed"""
        if not stats:
            embed = discord.Embed(
                title=f"📊 {user_name}'s Prediction Stats",
                description="No predictions made yet!",
                color=0x95a5a6
            )
            return embed
        ...
        embed = discord.Embed(
            title=f"📊 {user_name}'s Prediction Stats",
            description=f"**Rank #{stats['rank']}** • **{stats['total_points']} Total Points**",
            color=color,
            timestamp=datetime.now()
        )
        ...
        embed.add_field(
            ...
            value=f"**Predictions**: {stats['predictions_made']}\n"
                  f"**Correct**: {stats['correct_predictions']} ({stats['accuracy']:.1f}%)\n"
                  f"**Perfect Weeks**: {stats['perfect_weeks']}",
            inline=True
        )
        ...
        embed.add_field(
            name="🎯 By Category",
            value=f"**HOH**: {stats['hoh_correct']} correct\n"
                  f"**Veto**: {stats['veto_correct']} correct\n"
                  f"**Eviction**: {stats['eviction_correct']} correct",
            inline=True
        )
        
        # Streaks
        streak_text = f"**Current**: {stats['current_streak']}"
        ...
        if stats['recent_predictions']:
            recent_lines = []
            for pred in stats['recent_predictions'][:5]:
                emoji = "✅" if pred['correct'] else "❌"
                recent_lines.append(
                    f"{emoji} Week {pred['week']} {pred['type']}: {pred['prediction']}"
                )
    ...
    def _get_poll_emoji(self, poll_type: str) -> str:
        """Get emoji for poll type"""
        emojis = {
            'HOH': '👑',
            'VETO': '💎',
            'EVICTION': '🚪'
        }
        return emojis.get(poll_type, '🗳️')
