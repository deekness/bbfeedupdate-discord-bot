...
class BBDiscordBot(commands.Bot):
    ...
    # Add this to your BBDiscordBot class, before the setup_commands method

# Current Season 27 Houseguests - Update this list as needed
CURRENT_HOUSEGUESTS = [
    "Cory", "Jag", "Matt", "Bowie", "Felicia", "Blue", "America", "Cameron", 
    "Mecole", "Cirie", "Izzy", "Red", "Hisam", "Reilly", "Jared", "Luke"
]

class HouseguestSelectView(discord.ui.View):
    """View with dropdown for selecting houseguests"""
    
    def __init__(self, prediction_type: str, duration_hours: int, week_number: int = None):
        ...
        # Create dropdown with houseguests
        self.houseguest_select = HouseguestSelect(
            placeholder="Select houseguests for this poll...",
            min_values=2,  # At least 2 houseguests required
            max_values=min(len(CURRENT_HOUSEGUESTS), 25)  # Discord limit is 25
        )
        self.add_item(self.houseguest_select)
        
        # Add confirm button
        self.confirm_button = discord.ui.Button(
            label="Create Poll",
            style=discord.ButtonStyle.green,
            disabled=True  # Disabled until houseguests are selected
        )
        self.confirm_button.callback = self.confirm_poll
        ...
        self.cancel_button.callback = self.cancel_poll
        self.add_item(self.cancel_button)
    
    async def confirm_poll(self, interaction: discord.Interaction):
        """Create the poll with selected houseguests"""
        if not self.selected_houseguests:
            await interaction.response.send_message("Please select at least 2 houseguests first.", ephemeral=True)
            return
        
        # Auto-generate title and description based on prediction type
        pred_type_value = self.prediction_type
        if pred_type_value == "season_winner":
            title = "Season Winner"
            description = "Season 27"
            week_number = None
        elif pred_type_value == "weekly_hoh":
            title = f"Week {self.week_number} HOH"
            description = f"Week {self.week_number}"
            week_number = self.week_number
        elif pred_type_value == "weekly_veto":
            title = f"Week {self.week_number} Veto"
            description = f"Week {self.week_number}"
            week_number = self.week_number
        elif pred_type_value == "weekly_eviction":
            title = f"Week {self.week_number} Eviction"
            description = f"Week {self.week_number}"
            week_number = self.week_number
        ...
        try:
            ...
            prediction_id = bot.prediction_manager.create_prediction(
                title=title,
                description=description,
            ...
            embed = bot.prediction_manager.create_prediction_embed(prediction_data)
            ...
            await interaction.response.edit_message(
                content=f"Poll created successfully! ID: {prediction_id}\n"
                        f"**Title:** {title}\n**Description:** {description}\n"
                        f"**Selected Houseguests:** {', '.join(self.selected_houseguests)}",
                embed=embed,
                view=None  # Remove the view
            )
            
            # Announce in the main channel
            if bot.config.get('update_channel_id'):
                channel = bot.get_channel(bot.config.get('update_channel_id'))
                if channel:
                    announce_embed = bot.prediction_manager.create_prediction_embed(prediction_data)
    ...
    async def cancel_poll(self, interaction: discord.Interaction):
...
class HouseguestSelect(discord.ui.Select):
    """Dropdown select for choosing houseguests"""
    
    def __init__(self, placeholder: str, min_values: int, max_values: int):
        # Create options from houseguest list
        options = []
        for houseguest in CURRENT_HOUSEGUESTS:
            options.append(discord.SelectOption(
                label=houseguest,
                value=houseguest,
                emoji="🏠"  # House emoji for each houseguest
            ))
    ...
    async def callback(self, interaction: discord.Interaction):
        """Handle houseguest selection"""
        self.view.selected_houseguests = self.values
        
        # Enable the confirm button
        self.view.confirm_button.disabled = False
        
        # Update the message to show selected houseguests
        selected_text = f"**Selected Houseguests ({len(self.values)}):** {', '.join(self.values)}"
        
        await interaction.response.edit_message(
            content=f"Creating poll...\n{selected_text}\n\nClick 'Create Poll' to confirm or select different houseguests.",
            view=self.view
        )

# Now update your createpoll command in setup_commands():
...
async def createpoll_slash(interaction: discord.Interaction, 
    ...
    """Create a prediction poll with houseguest dropdown selection"""
    try:
        ...
        # Create the houseguest selection view
        view = HouseguestSelectView(pred_type_value, duration_hours, week_number)
        
        # Send the selection interface
        await interaction.response.send_message(
            f"Creating a **{prediction_type.name}** poll that will run for **{duration_hours} hours**.\n"
            f"Please select the houseguests to include in this poll:",
            view=view,
            ephemeral=True
        )
