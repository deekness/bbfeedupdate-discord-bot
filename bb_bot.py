...
class BBDiscordBot(commands.Bot):
    def __init__(self):
        ...
        self.batch_buffer = []
        self.batch_time_window = 60  # seconds to wait before sending batch
        self.batch_max_items = 10    # max updates per batch
    ...
    async def send_batch_to_channel(self, updates: List[BBUpdate]):
        channel_id = self.config.get('update_channel_id')
        if not channel_id:
            logger.warning("Update channel not configured")
            return
        try:
            ...
            if not updates:
                return
            batch_message = "**Batch of Big Brother Live Feed Updates:**\n\n"
            for update in updates:
                time_str = update.pub_date.strftime("%I:%M %p %Z")
                batch_message += f"- {time_str} - {update.title}\n"
            await channel.send(batch_message)
            logger.info(f"Posted batch of {len(updates)} updates to channel {channel_id}")
        except Exception as e:
            logger.error(f"Error sending batch updates to channel: {e}")

    async def batch_sender_loop(self):
        while not self.is_shutting_down:
            if self.batch_buffer:
                batch_to_send = self.batch_buffer.copy()
                self.batch_buffer.clear()
                await self.send_batch_to_channel(batch_to_send)
            await asyncio.sleep(self.batch_time_window)
    ...
    async def check_rss_feed(self):
        try:
            ...
            if feed.bozo:
                logger.warning("Feed parser bozo detected, skipping this check")
                return
            ...
            for update in new_updates:
                ...
                # Hybrid: Send immediately if importance >= 5, else buffer for batch
                if importance >= 5:
    ...
    @app_commands.command(name="summary", description="Get a summary of today's updates.")
    async def summary(self, interaction: discord.Interaction):
        try:
            now = datetime.now()
            start_of_day = now.replace(hour=6, minute=1, second=0, microsecond=0)
            if now < start_of_day:
            ...
            if not updates:
                ...
                return
            summary = "\n".join(f"- {u.pub_date.strftime('%I:%M %p')}: {u.title}" for u in updates)
            await interaction.response.send_message(f"**Today's Updates:**\n{summary}", ephemeral=True)
        except Exception as e:
            logger.error(f"Error in summary command: {e}")
            await interaction.response.send_message("Failed to get summary.", ephemeral=True)

    @app_commands.command(name="bbhelp", description="Show bot commands and usage.")
    async def bbhelp(self, interaction: discord.Interaction):
        help_text = (
            "**Big Brother Bot Commands:**\n"
            "/summary - Show today's updates summary\n"
            "/bbhelp - Show this help message\n"
            # Add other commands descriptions here as needed
        )
        await interaction.response.send_message(help_text, ephemeral=True)

    # Register commands explicitly on setup_hook
    async def register_commands(self):
        self.tree.add_command(self.summary)
        self.tree.add_command(self.bbhelp)
        await self.tree.sync()

    async def close(self):
        logger.info("Closing bot and cleaning up...")
        self.is_shutting_down = True
        if self.batch_task:
...
async def main():
    ...
    bot.batch_task = asyncio.create_task(bot.batch_sender_loop())
